...
public class MemInfoMetricCollectorTest {
    ...
    @Spy MemInfoMetricCollector compactMeminfoMetricCollector;
    ...
    public void setup() throws Exception {
        ...
        doNothing()
                .when(compactMeminfoMetricCollector)
                .saveProcessOutput(anyString(), any(File.class));

        doReturn(tempFolder.newFolder()).when(compactMeminfoMetricCollector).createTempDir();
    }

    @Test
    public void testCollect() throws Exception {
        DeviceMetricData runData = new DeviceMetricData(mContext);
        when(compactMeminfoMetricCollector.getFileSuffix()).thenReturn("1");

        compactMeminfoMetricCollector.collect(runData);
        ...
        assertTrue(metricsCollected.containsKey("compact-meminfo-1"));
    }
}
